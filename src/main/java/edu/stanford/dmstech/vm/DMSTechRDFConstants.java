...
public Resource oacTargetType;
	...
	textAnnotationClass = initializingModel.createResource("http://dms.stanford.edu/ns/TextAnnotation");
	transactionClass = initializingModel.createResource(STANFORD_NAMESPACE + "Transaction");
	
	// http properties and types
	httpMethod = initializingModel.createProperty (HTTP_NAMESPACE, "mthd");
	httpPostResource = initializingModel.createResource(HTTP_NAMESPACE + "POST");
			
	cntRestProperty = initializingModel.createProperty(CNT_NAMESPACE, "rest");
	cntCharEncProperty = initializingModel.createProperty(CNT_NAMESPACE, "characterEncoding");
	cntAsTxtType = initializingModel.createResource(CNT_NAMESPACE + "ContentAsText");
	
	// ORE properties and types
	oreDescribes = initializingModel.createProperty(ORE_NAMESPACE, "describes");
	oreAggregates = initializingModel.createProperty(ORE_NAMESPACE, "aggregates");
	oreResourceMapClass = initializingModel.createResource(ORE_NAMESPACE + "ResourceMap");
	oreAggregationClass = initializingModel.createResource(ORE_NAMESPACE + "Aggregation");

	// OAC properties and types
	oacHasBodyProperty = initializingModel.createProperty(OAC_NAMESPACE, "hasBody");
	oacHasTargetProperty = initializingModel.createProperty(OAC_NAMESPACE, "hasTarget");
	oacAnnotationType = initializingModel.createResource(OAC_NAMESPACE + "Annotation");
	oacTextAnnotationType = initializingModel.createResource(OAC_NAMESPACE + "TextAnnotation");
	oacBodyType = initializingModel.createResource(OAC_NAMESPACE + "Body");
	oacTargetType = initializingModel.createResource(OAC_NAMESPACE + "Target");
